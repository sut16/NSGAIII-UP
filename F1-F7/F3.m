...
% <multi/many> <real> 
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            SumG_odd=sum(1-exp(-abs(g_odd)),2);
            
            norm_even=sqrt(sum(repmat(PopDec_even,cou,1).^2,2));
            %a=1.-exp(-sqrt(sum(g_even.^2,2)));
            SumGandSin_evn=sum(1-exp(-abs(g_even)),2);

            f1=PopDec(:,1)+2./norm_odd.*SumG_odd;
            f2=zeros(cou,1);
            for index=1:cou
                if(PopDec(index,1)<=0.5)
                    f2(index)=1-8*PopDec(index,1)^4+2/norm_even(index)*SumGandSin_evn(index);
                else
                    f2(index)=8*(1-PopDec(index,1))^4+2/norm_even(index)*SumGandSin_evn(index);
                end
            end
            PopObj = [f1,f2];
        end
